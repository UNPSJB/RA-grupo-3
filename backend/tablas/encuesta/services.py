...
from tablas.encuestas.models import Encuesta
from tablas.encuestas import schemas
...
def listar_encuestas(db: Session) -> List[Encuesta]:
    stmt = select(Encuesta).options(selectinload(Encuesta.secciones).selectinload(schemas.Seccion.preguntas))
