...
import type { SeriesDescriptor, ChartDatum } from "./charData.ts";
...
const Grafico: React.FC<GraficoProps> = ({ data, series }) => {
  // El payload de la leyenda se genera dinámicamente
        ...
        >
          ...
          <Tooltip
            formatter={(value, name) => {
              ...
              // Busca en las series dinámicas
              const match = series.find((item) => item.key === serieKey);
              return [value, match ? match.label : serieKey];
            }}
          />
          <Legend payload={legendPayload} />
          {/* Mapea las series dinámicas */}
